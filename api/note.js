...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      // Delete notes for fridge - supports both individual and global deletion
      const { fridgeId, deleteAll, noteId } = req.body;
      ...
      try {
        if (deleteAll) {
          // DELETE ALL NOTES - existing logic
          const blobPrefix = `fridge-${fridgeId}`;
          console.log('Deleting ALL notes - blobs with prefix:', blobPrefix);
          ...
          // DELETE INDIVIDUAL NOTE - new logic
          if (!noteId) {
            return res.status(400).json({ error: 'Note ID is required for individual deletion' });
          }
          
          console.log('Deleting individual note:', noteId, 'from fridge:', fridgeId);
          
          // Fetch existing notes
          const existingData = await fetchExistingNotes(fridgeId);
          
          if (!existingData.notes || existingData.notes.length === 0) {
            return res.json({ 
              success: true,
          ...
          const originalCount = existingData.notes.length;
          const updatedNotes = existingData.notes.filter(note => {
            // Check both id and timestamp for compatibility
            const noteIdentifier = note.id || note.timestamp.toString();
            return noteIdentifier !== noteId && note.timestamp.toString() !== noteId;
          });
          
          if (updatedNotes.length === originalCount) {
          ...
          console.log(`Removed note. Before: ${originalCount}, After: ${updatedNotes.length}`);
          
          if (updatedNotes.length === 0) {
            // No notes left - delete the entire blob
            const blobPrefix = `fridge-${fridgeId}`;
            const { blobs } = await list({ prefix: blobPrefix });
            
            for (const blob of blobs) {
              await del(blob.pathname);
              console.log('Deleted empty blob:', blob.pathname);
            }
            
            return res.json({ 
              success: true, 
              message: 'Last note deleted, fridge cleared',
              fridgeId: fridgeId,
              remainingNotes: 0
            });
          } else {
            // Save updated notes array back to blob
            const blobKey = `fridge-${fridgeId}.json`;
            await put(blobKey, JSON.stringify({ 
              notes: updatedNotes,
              lastUpdated: Date.now(),
