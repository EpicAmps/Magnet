...
export async function loadNotes(fridgeId) {
  ...
  try {
    ...
    const cacheSafeUrl = `${metadata.downloadUrl}?t=${Date.now()}`;
    ...
    try {
      ...
      console.warn("Failed to parse blob JSON, returning empty list", {
        fridgeId,
    ...
    return {
      ...
      lastUpdated:
        payload.lastUpdated || metadata.uploadedAt?.getTime() || Date.now(),
...
export async function listFridgeBlobs(prefix = "fridge-") {
